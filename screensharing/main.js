...
class ScreenShareMainHook {
    /**
     * Create ScreenShareMainHook linked to confabboxWindow.
     *
     * @param {BrowserWindow} confabboxWindow - BrowserWindow where confabbox api is loaded.
     ...
     */
    constructor(confabboxWindow, identity, osxBundleId) {
        this._confabboxWindow = confabboxWindow;
        ...
        this._confabboxWindow.on('closed', () => {
    ...
    _onScreenSharingEvent(event, { data }) {
        switch (data.name) {
            ...
            case SCREEN_SHARE_EVENTS.STOP_SCREEN_SHARE:
                this._confabboxWindow.webContents.send(SCREEN_SHARE_EVENTS_CHANNEL, { data });
                break;
            default:
 ...
 *
 * @param {BrowserWindow} confabboxWindow - the BrowserWindow object which displays Confabbox
 * @param {string} identity - Name of the application doing screen sharing, will be displayed in the
 * screen sharing tracker window text i.e. {identity} is sharing your screen.
 * @param {string} bundleId- OSX Application BundleId
 */
module.exports = function setupScreenSharingMain(confabboxWindow, identity, osxBundleId) {
    return new ScreenShareMainHook(confabboxWindow, identity, osxBundleId);
};
