 ...
 * @param {BrowserWindow} confabboxWindow - the BrowserWindow object which
 * displays Confabbox.
 * @private
 */
function _attachEvents(confabboxWindow) {
    browserWindow = confabboxWindow;
...
/**
 * The error result to pass back to Confabbox.
 * @param id - Id of the request.
 ...
 * displays Confabbox
 */
module.exports = function setupPowerMonitorMain(confabboxWindow) {
    if (app.isReady()) {
        _attachEvents(confabboxWindow);
    } else {
        app.on('ready', () => _attachEvents(confabboxWindow));
    }
